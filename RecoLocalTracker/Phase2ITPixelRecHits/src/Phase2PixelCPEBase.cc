...
//  
//-----------------------------------------------------------------------------
Phase2PixelCPEBase::Phase2PixelCPEBase(edm::ParameterSet const & conf, 
    ...
    magfield_(mag), geom_(geom), ttopo_(ttopo)  
{
  ...
  LogDebug("Phase2PixelCPEBase") <<" LA constants - "
...
//-----------------------------------------------------------------------------
void Phase2PixelCPEBase::fillDetParams()
{
  ...
  for (unsigned i=0; i!=m_detectors;++i) {
    ...
    //cout<<" in Phase2PixelCPEBase - in det "<<thePart<<endl; //dk
    ...
    LogDebug("Phase2PixelCPEBase") << "***** PIXEL LAYOUT *****" 
			     << " thePart = " << p.thePart
...
//-----------------------------------------------------------------------------
void
Phase2PixelCPEBase::setTheClu( DetParam const & theDetParam, ClusterParam & theClusterParam ) const 
{
...
//-----------------------------------------------------------------------------
//float 
//Phase2PixelCPEBase::estimatedAlphaForBarrel(float centerx) const
//{
...
// The angles from dets are calculated internaly in the Phase2PixelCPEInitial class
//-----------------------------------------------------------------------------
...
// better way.
//-----------------------------------------------------------------------------
bool Phase2PixelCPEBase::isFlipped(DetParam const & theDetParam) const 
{
...
//------------------------------------------------------------------------
Phase2PixelCPEBase::DetParam const & Phase2PixelCPEBase::detParam(const GeomDetUnit & det) const {
  auto i = det.index();
...
//-----------------------------------------------------------------------------
LocalVector 
Phase2PixelCPEBase::driftDirection(DetParam & theDetParam, GlobalVector bfield ) const {
...
LocalVector 
Phase2PixelCPEBase::driftDirection(DetParam & theDetParam, LocalVector Bfield ) const {
  ...
  if(LocalPrint) cout<<" in Phase2PixelCPEBase:driftDirection - "<<langle<<" "<<Bfield<<endl; //dk
  ...
  LogDebug("Phase2PixelCPEBase") << " The drift direction in local coordinate is "  << dd   ;
...
//-----------------------------------------------------------------------------
void
Phase2PixelCPEBase::computeLorentzShifts(DetParam & theDetParam) const {

  //cout<<" in Phase2PixelCPEBase:computeLorentzShifts - "<<driftDirection_<<endl; //dk
  ...
  //cout<<" in Phase2PixelCPEBase:computeLorentzShifts - "
  //<<lorentzShiftInCmX_<<" "
  //<<lorentzShiftInCmY_<<" "
  //<<endl; //dk
  
  LogDebug("Phase2PixelCPEBase") << " The drift direction in local coordinate is " 
...
Phase2PixelCPEBase::rawQualityWord(ClusterParam & theClusterParam) const
{
