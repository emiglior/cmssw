    ...
    GeomDetType::SubDetector det)
{
  ...
  for (auto const& i : gdv) {
    ...
    if (thePixelDetTypeMap.find(detName) == thePixelDetTypeMap.end()) {
      std::unique_ptr<const Bounds> bounds(i->bounds());
      // EM for now I am just assigning fixed values... Which numbers are to be assigned will depend on the actual implementation of PixelPhase2TopologyBuilder()
      int BIG_PIX_PER_ROC_X = (i->bigPixelsx()) ? 2 : 0; // in x direction, rows
      int BIG_PIX_PER_ROC_Y = (i->bigPixelsy()) ? 1 : 0; // in y direction, cols
