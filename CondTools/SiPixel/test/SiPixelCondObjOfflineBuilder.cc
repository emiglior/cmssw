...
void
SiPixelCondObjOfflineBuilder::analyze(const edm::Event& iEvent, const edm::EventSetup& iSetup)
{
   using namespace edm;
...
//    int myrow = 159;
   ...
   for(TrackerGeometry::DetContainer::const_iterator it = pDD->dets().begin(); it != pDD->dets().end(); it++){
	 ...
	 rmsGainWork = rmsGainFPix_;
       ...
       std::cout << " PixelIndices called by SiPixelCondObjOfflineBuilder::analyze() #1" << std::endl;
...
bool SiPixelCondObjOfflineBuilder::loadFromFile() {
  ...
  if ( in_file.eof() != 0 ) {
    edm::LogError("SiPixelCondObjOfflineBuilder")<< in_file.eof() << " " << in_file.gcount() << " "
  ...
  for(int i=0;i<(52*80);i++)  {
    ...
    std::cout << " PixelIndices called by SiPixelCondObjOfflineBuilder::loadFromFile()" << std::endl;
